...
def log_energia_pkl(arquitetura, n_mfccs, seed, energia_kwh):
    pkl_path = os.path.join("resultados", "consumo_energia.pkl")
    os.makedirs("resultados", exist_ok=True)
    dados_energia = {}
    ...
    if os.path.exists(pkl_path):
        try:
            with open(pkl_path, 'rb') as f:
                dados_energia = pickle.load(f)
        except (EOFError, FileNotFoundError): pass

    if arquitetura not in dados_energia: dados_energia[arquitetura] = {}
    if n_mfccs not in dados_energia[arquitetura]: dados_energia[arquitetura][n_mfccs] = {}
    
    dados_energia[arquitetura][n_mfccs][seed] = float(f"{energia_kwh:.6f}")
    with open(pkl_path, 'wb') as f:
        pickle.dump(dados_energia, f)
...
def train_model(processed_path, n_classes, arquitetura, qtdEpocas, flexMfccs, seed):
    ...
    energia_kwh = calcular_energia_kwh(elapsed_time)
    ...
    log_energia_pkl(arch, flexMfccs, seed, energia_kwh)
